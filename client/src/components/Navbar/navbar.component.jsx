...
const NavLg = ({ setIsSignupOpen }) => {
  const navigate = useNavigate();
  const { user, setUser } = useContext(UserContext);

  const handleLogout = async () => {
    const response = await backendApi.post("/auth/signout");
        ...
        <div className="flex items-center gap-3">
          ...
          {user && user.name ? (
            <Menu as="div" className="relative inline-block text-left">
              <MenuButton className="px-4 py-2 text-gray-200 text-sm flex items-center cursor-pointer hover:text-white">
                {user.name}
              </MenuButton>
              <MenuItems className="absolute right-0 mt-2 w-48 origin-top-right bg-white divide-y divide-gray-200 rounded-md shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                <div className="py-1">
                  <MenuItem>
                    {({ focus }) => (
                      <a
